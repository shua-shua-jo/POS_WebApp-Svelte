...
<div class="container">
	...
	<div class="wrapper-container">
		<div class="wrapper">
			<div class="header">
				<div class="text">User Info</div>
			</div>
			<div class="content">
				{#each data.user as data, i}
					{#if i < 1}
						{@const user = data.usersTable}
						<div class="user-info">
							<b>Request No. {user.id}</b>
						...
						{user.request_available}
						{user.total_price}
					{/if}
					{@const request = data.requestsTable}
					{request.id}
					{request.document}
					{request.price}
				{:else}
